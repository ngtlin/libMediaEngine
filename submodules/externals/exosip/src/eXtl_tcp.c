...
#undef USE_SERVER_SOCKET /*who cares needs a server socket on a mobile client ??*/
#endif
...
static int tcp_tl_open(void)
{
	...
	for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
		...
		tmp=1;
		res=setsockopt(sock,SOL_SOCKET,SO_REUSEADDR,&tmp,sizeof(tmp));
		if (res!=0){
			OSIP_TRACE(osip_trace
					   (__FILE__, __LINE__, OSIP_ERROR, NULL,
						"Cannot set socket SO_REUSEADDR node:%s family:%d %s\n",
	...
	if (FD_ISSET(tcp_socket, osip_fdset)) {
		...
		if (sock < 0) {
			...
			if (status==EBADF)
			{
				...
				/* this error occurs on iOS, when returning from background mode.
				 * We need to re-open the server socket, but make sure that all child of the server socket are closed
				 * so that the port is free.
				 */
				memset(&ai_addr, 0, sizeof(struct sockaddr_storage));
				if (tcp_socket > 0){
					closesocket(tcp_socket);
					for (i = 0; i < EXOSIP_MAX_SOCKETS; i++) {
						if (tcp_socket_tab[i].socket > 0 && tcp_socket_tab[i].is_server){
							_tcp_tl_close_sockinfo(&tcp_socket_tab[i]);
						}
					}
				}
				tcp_tl_open();
...
static int _tcp_tl_is_connected_with_timeout(int sock, int millis) {
	int res;
	struct timeval tv;
	fd_set wrset;
	int valopt;
...
{
	return _tcp_tl_is_connected_with_timeout(sock,SOCKET_TIMEOUT);
}
...
static int _tcp_tl_check_connected()
{
	...
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (tcp_socket_tab[pos].invalid){
			/*this socket was set to invalid, close it */
			OSIP_TRACE(osip_trace
							(__FILE__, __LINE__, OSIP_INFO2, NULL,
							"_tcp_tl_check_connected: Socket node invalidated:%s:%i, socket %d [pos=%d], family:%d\n",
	...
	for (curinfo = addrinfo; curinfo; curinfo = curinfo->ai_next) {
		...
		if (res < 0) {
			...
			if (ex_errno != EINPROGRESS) {
				...
				res = _tcp_tl_is_connected_with_timeout(sock, SOCKET_PROGRESS_TIMEOUT);
				if (res > 0) {
					OSIP_TRACE(osip_trace
							(__FILE__, __LINE__, OSIP_INFO2, NULL,
									"WARNING, socket node:%s, socket %d [pos=%d], family:%d, still in progressr:, closing.\n",
...
_tcp_tl_send_sockinfo (struct _tcp_sockets *sockinfo, const char *msg, int msglen)
{
	...
	while (1) {
		i=0;
#ifdef ANDROID /*workaround to force REGISTRAR to be issued with certain Android device*/
		if(msglen >100) {
			i = send(sockinfo->socket, (const void *) msg, 100, 0);
		}
#endif
		i +=  send(sockinfo->socket, (const void *) msg+i, msglen-i, 0);
		if (i < 0) {
			int status = ex_errno;
			if (is_wouldblock_error(status)) {
				struct timeval tv;
...
static int tcp_tl_reset(void){
	int pos;
	for (pos = 0; pos < EXOSIP_MAX_SOCKETS; pos++) {
		if (tcp_socket_tab[pos].socket!=0)
			tcp_socket_tab[pos].invalid=1;
	}
	return 0;
}
